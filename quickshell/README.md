...
- **The bar now recolors from the wallpaper via matugen.** matugen renders `templates/colors.json` into `~/.cache/quickshell/colors.json` on every wallpaper change (Rofi keybind picker and in-shell Wallpapers tab) and Quickshell's `FileView` watches that file, so the bar and all panels recolor live — no restart. The bar pill is a dark shade of the wallpaper's color (`primary_container`), so it stays readable (~7:1 contrast) instead of turning white. `prefer = "darkness"` + `contrast = -0.5` in the matugen config mean the palette is built from the wallpaper's **dark** regions and pulled deep/desaturated ("black-green" tones), so white/bright parts of an image never drive the theme. Settings → Appearance has a master **"Wallpaper colors"** switch (whole shell on/off) plus an **"Accent from wallpaper"** switch; clicking an accent swatch flips the latter off automatically so manual accents actually take effect.
- **Dashboard opening is hover-INTENT on the clock only.** The whole center strip no longer triggers it — only the clock pill does, and only after the pointer rests on it for 180ms, so sweeping the mouse across the bar never pops the overview open. Clicking the clock toggles it pinned. (Previous behavior: any hover anywhere between the icon groups opened it.)
- **Two settings that did nothing now work**: "Bar margin & gap" (`barGap` was never declared, so the slider errored and did nothing — now it's real, drives the bar's margins *and* keeps every popout panel flush with the same value, scaled by UI scale) and "Bar background opacity" (declared but never applied — now applied to the pill color).
- **Dashboard riced**: soft wallpaper-accent halo + dark drop shadow behind the card, slide/fade/scale entrance animation, accent-pill highlight on the active/hovered tab, and an accent hairline that fades out at both ends under the tab bar. The bar gets the same fading accent hairline along its bottom edge, and the clock pill now tints with the accent on hover.

- **The whole center bar area is now the hover trigger**, not just the small clock pill — hover anywhere in the free space between the left and right icon groups to open the dashboard, much easier to hit than the pill alone. *(Superseded: see the hover-intent bullet above — the trigger is the clock pill again, with a dwell delay.)*
